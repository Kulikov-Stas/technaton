...
class ControllerNewsBlogArticle extends Controller {
	...
	public function index() {
		...
		if ($article_info) {
            ...
            $data['date'] = ($date_format ? explode(' ', date($date_format, strtotime($article_info['date_available'])))[0] : false);
            $data['date_modified'] = ($date_format ? date($date_format, strtotime($article_info['date_modified'])) : false);

            $data['speed'] = round(mb_strlen(preg_replace('/[\s]{2,}/', '', strip_tags($data['description'])), 'UTF-8') / 150);
