...
class ModelNewsBlogArticle extends Model {
	...
	public function getArticle($article_id) {
		...
		if ($query->num_rows) {
			return array(
                ...
                'speed'            => round(mb_strlen(
                    preg_replace('/[\s]{2,}/', '', strip_tags(
                        html_entity_decode(
                            $query->row['description'], ENT_QUOTES, 'UTF-8')
                        )
                    ), 'UTF-8') / 150)
			);
		} else {
    ...
    public function getArticlesByName($data = array()) {
        ...
        $sql .= ' AND (ad.name LIKE "%' . $this->db->escape($data['name']) . '%"
                        OR ad.description LIKE "%' . $this->db->escape($data['name']) . '%") ';
        ...
        foreach ($query->rows as $result) {
            ...
            $article_data[$result['article_id']]['image'] = $this->getArticleImages($result['article_id']);
        }
        return $article_data;
    }
	...
	public function getArticleRelated($article_id) {
		...
		foreach ($query->rows as $result) {
            ...
            $article_data[$result['article_id']]['image'] = $this->getArticleImages($result['related_id']);
		}
		return $article_data;
	}
