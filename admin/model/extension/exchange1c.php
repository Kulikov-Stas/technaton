...
//		// Полное наименование из 1С в товар
	 ...
	 */
 	private function setProduct(&$data, $type = 'model') {
		...
		// Если не найден товар...
 		if (!$data['product_id']) {
 		    ...
 		    // у нас есть комплектующие с одинаковым id - они хотя и одинаковые
            // НО принадлежат к разным моделям например винтик может быть
            // у 10 разных устройств поэтому вместо апдейта добавляем
            if ($type == 'model') {
	...
	private function addManufacturer(&$manufacturer_data) {
		...
		//$this->query("INSERT INTO `" . DB_PREFIX . "manufacturer` SET" . implode(", ", $sql));
        $this->query('INSERT INTO `' . DB_PREFIX . 'manufacturer` SET name = "' . $manufacturer_data['name'] . '", image = "' . $manufacturer_data['image'] . '"');
	 ...
	 */
	private function setManufacturer($data, $manufacturer_guid = '', $model = false) {

		$manufacturer_data = array();
		$manufacturer_data['name']			= (string)$data['name'];
        ...
        $manufacturer_data['image']         = $data['image'];
		...
		if ($manufacturer_guid) {
			...
			$this->log('Поиск (производителя) изготовителя по 1C Ид ' . $query->num_rows);
			if ($query->num_rows == 0) {
                $query = $this->query('SELECT manufacturer_id FROM `' . DB_PREFIX . 'manufacturer` WHERE name LIKE "' . $this->db->escape($manufacturer_data['name']) . '"' );
                if ($query->num_rows > 0 && !$model) {
                    $this->log('Поиск (производителя) изготовителя найден! ' . $query->num_rows);
                    return true;
                }
            }
		} else {
        ...
        //$this->log('$manufacturer_data["manufacturer_id"]: ' . $manufacturer_data['manufacturer_id']);
	 ...
	 */
	private function getProductManufacturerId($product, $product_guid) {
        ...
        if ($product->ИдБренда && !in_array($product->ИдБренда, $this->brands)) {
            return $this->setManufacturer($product->ИдБренда, $product_guid, true);

        }
        $this->brands[] = $product->ИдБренда;
		$this->log('$product->ИдБренда: ' . $product->ИдБренда);
...
//			$data['mpn']			= $data['product_guid'];
    ...
    private function parseAccessories($xml, $classifier) {
        ...
        foreach ($xml->Комплектующая as $product){
...
//			$data['mpn']			= $data['product_guid'];
            ...
            if ($product->Цена) {
                ...
                if ($this->ERROR) return false;
                $this->log('price: ' . $data['price']);
            } // $product->Цена

            // Добавляем или обновляем товар в базе
            $this->setProduct($data, 'accessories');
            if ($this->ERROR) return false;
    ...
    private function parseBrands($xml) {
        ...
        foreach ($xml->Бренд as $brand){

            $data = array();

            // Получаем Ид товара и характеристики
            $guid_full = explode("#", (string)$brand->Ид);
            $data['product_guid']	= $guid_full[0];
            $this->log('brand_id: ' . $data['product_guid']);
            $data['feature_guid']	= isset($guid_full[1]) ? $guid_full[1] : '';
            $data['product_id'] 	= 0;
//			$data['mpn']			= $data['product_guid'];
            $data['name']			= htmlspecialchars((string)$brand->Наименование);
            $this->log('brand_name: ' . $data['name']);

            $this->log("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -", 2);
            if ($data['feature_guid']) {
                $this->log("Бренд: Ид: '" . $data['product_guid'] . "', ХАРАКТЕРИСТИКА Ид: '" . $data['feature_guid'] . "'");
            } else {
                $this->log("Бренд: Ид: '" . $data['product_guid'] . "'");
            }

            // Описание товара в текстовом формате, нужна опция если описание в формате HTML
            if ($brand->Описание && $this->config->get('exchange1c_import_product_description') == 1)	{
                if ($this->config->get('exchange1c_description_html') == 1) {
                    $data['description']	=  nl2br(htmlspecialchars((string)$brand->Описание));
                    $this->log('description: ' . $data['description']);
                } else {
                    $data['description'] 	= (string)$brand->Описание;
                }
            }
            ...
            if ($this->config->get('exchange1c_product_images_import_mode') != 'disable') {
                ...
                if ($brand->Картинка) {
                    ...
                    $images = isset($data['images']) ? $data['images'] : array();
                    $data['images'] = $this->parseImages($brand->Картинка, $images, $description);
                ...
                // Картинки, только для товара (CML 2.04)
                if ($brand->ОсновнаяКартинка) {

                    $images = isset($data['images']) ? $data['images'] : array();
                    $data['images'] = $this->parseImages($brand->ОсновнаяКартинка, $images, $description);
                    ...
                    while (!empty($brand->$var)) {

                        $data['images'] = $this->parseImages($brand->$var, $data['images'], $description);
                        if ($this->ERROR) return false;
            ...
            /*foreach ($data as $key => $value) {
                $this->log('По брендам: ' . $key . ' ' . $value);
            }*/

            // Если включено обновление производителя
            if ($this->config->get('exchange1c_import_product_manufacturer') == 1) {

                $manufacturer_id = $this->getProductManufacturerId($brand, $data['product_guid']);
                $this->log('$manufacturer_id: ' . $manufacturer_id);
                if ($manufacturer_id) {
                    $data['manufacturer_id'] = $manufacturer_id;
                } else {
                    $data['manufacturer_id'] = $this->setManufacturer($data, $data['product_guid']);
                    $this->log("> Производитель: '" . $data['name'] . "', id: " . $data['product_guid'] ,2);
                }
            }
            ...
            // Прерывание обмена
            if (file_exists(DIR_CACHE . 'exchange1c/break')) {
                $this->ERROR = "parseBrands() - file exists 'break'";
                return false;
            }
        } // foreach
	...
	private function parseDirectory($xml, $classifier) {
        ...
        if ($xml->Классификатор->Бренды) {
            // Загрузка брендов
            $this->parseBrands($xml->Классификатор->Бренды);
            if ($this->ERROR) return false;
        }
...
//		$requisites['Склад']						= 'Основной склад'
	...
	public function importFile($importFile, $type) {
		...
		if ($xml->Каталог->Классификатор) {
			$this->log(">>>>>>>>>>>>>>>>>>>> ЗАГРУЗКА КЛАССИФИКАТОРА <<<<<<<<<<<<<<<<<<<<",2);
			$classifier = $this->parseClassifier($xml->Каталог->Классификатор);
			if ($this->ERROR) return $this->error();
			//unset($xml->Каталог->Классификатор);
		} else {
			// CML 2.08 + Битрикс
			$classifier = array();
		}
